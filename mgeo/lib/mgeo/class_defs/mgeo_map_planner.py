...
class MgeoMapPlanners:
    ...
    def append_map(self, add_path):
        ...
        Variable    : path_name
        """ 
        #Logger.log_trace('Add Mgeo data : {}'.format(add_path.split('/')[-1]))
        
        # For secure purpose
        if not self.mgeo_maps_dict:
            #Logger.log_error('Please load the map data first before appending')
            return
        else:
    ...
    def clear_all_map(self):
        ...
        """
        #Logger.log_trace('Clear all the MGeo Map File Y or N')
